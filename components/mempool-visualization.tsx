  ...
  const getTransactionValue = (tx: any) => {
    ...
    if (tx.vout && Array.isArray(tx.vout) && tx.vout.length > 0) {
      const total = tx.vout.reduce((sum: number, output: any) => {
        ...
        if (
          output.addresses &&
          typeof output.addresses === "object" &&
          output.addresses.amount
        ) {
    ...
    if (tx.vin && Array.isArray(tx.vin) && tx.vin.length > 0) {
      const total = tx.vin.reduce((sum: number, input: any) => {
        ...
        if (
          input.addresses &&
          typeof input.addresses === "object" &&
          input.addresses.amount
        ) {
...
Fee: ${tx.fee} AEGS`
              ...
              onClick={() =>
                tx.txid && (window.location.href = `/tx/${tx.txid}`)
              }
            >
              ...
              <span className="hidden sm:inline text-white text-xs font-bold drop-shadow-sm">
                {valueText}
              </span>
            </div>
          );
          ...
          {transactions.length === 0 ? (
            ...
            <>
              ...
              <p className="text-sm text-muted-foreground mt-4">
                Each block represents a transaction with its AEGS value. Hover
                to see details, click to view transaction.
              </p>
            </>
          )}
        </CardContent>
