...
const MONGODB_URI =
  process.env.MONGODB_URI || "mongodb://localhost:27017/explorerdb";
...
async function createIndexes() {
  try {
    ...
    await db
      ...
      .createIndex({ a_id: 1, txid: 1 }, { unique: true });
    ...
    await db
      ...
      .createIndex({ blockindex: 1 }, { unique: true });
  ...
  try {
    ...
    if (blockHeight >= 4) {
      ...
      while (remainingBlocks > 0) {
        ...
        if (
          currentHalvingBlock % halvingInterval === 0 &&
          remainingBlocks > 0
        ) {
          currentReward /= 2;
        }
    ...
    console.log(
      `Calculated supply at block ${blockHeight}: ${totalSupply} AEGS`,
    );
    return totalSupply;
  } catch (error) {
    ...
    for (const input of tx.vin) {
      if (input.coinbase) {
        ...
        try {
          ...
          console.error(
            `Error processing input ${input.txid}:${input.vout}:`,
            error.message,
          );
        }
      }
...
async function createAddressTx(address, txid, amount, blockHeight) {
  try {
    ...
    const existingLink = await db
      .collection("addresstxes")
      .findOne({ a_id: address, txid: txid });
    ...
    console.error(
      `Error creating address-tx link ${address}:${txid}:`,
      error.message,
    );
  }
}
...
async function updateNetworkStats(blockHeight) {
  try {
    ...
    const [blockchainInfo, networkInfo, miningInfo, networkHashPS] =
      await Promise.all([
        getBlockchainInfo(),
        getNetworkInfo(),
        getMiningInfo(),
...
async function syncMempool() {
  try {
    ...
    console.log(
      `Mempool sync completed. ${mempoolTxs.length} transactions in mempool.`,
    );
  } catch (error) {
...
async function initializeDatabase() {
  try {
    ...
    const coinStats = await db
      .collection("coinstats")
      .findOne({ coin: "Aegisum" });
...
async function syncBlockchain() {
  try {
    ...
    console.log(
      `Current blockchain height: ${currentHeight}, Last synced height: ${lastSyncedHeight}`,
    );
