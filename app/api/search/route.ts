...
export async function GET(request: Request) {
  ...
  try {
    ...
    const txWithBlockHash = await db
      .collection("txes")
      .findOne({ blockhash: query });
    if (txWithBlockHash) {
      return NextResponse.json({ type: "block", id: query });
    }
