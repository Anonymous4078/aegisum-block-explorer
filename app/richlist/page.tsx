...
export default async function RichListPage() {
  const [richList, networkStats] = await Promise.all([
    getRichList(100),
    getNetworkStats(),
  ]);
    ...
    <main className="container mx-auto px-4 py-6 max-w-7xl">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">
          Top 100 richest Aegisum addresses by balance
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Total Supply: {formatNumber(networkStats.supply)} AEGS
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            ...
            <TableBody>
              {richList.map((address, index) => {
                const percentOfSupply =
                  (address.balance / (networkStats.supply * 100000000)) * 100;
                return (
                  <TableRow key={address.a_id}>
                    ...
                    <TableCell className="text-right">
                      {formatNumber(address.balance / 100000000)} AEGS
                    </TableCell>
